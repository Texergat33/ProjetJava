...
public abstract class NewDAO extends AbstractDAO {
	...
	public static GamingMap getLevelByID(int levelID) throws SQLException {
		...
		if (callStatement.execute()) {
			...
			if (result.first()) {
				gamingMap = new GamingMap(result.getInt(IDColumnIndex),
						result.getInt(WidthColumnIndex), result.getInt(HeightColumnIndex), result.getInt(DiamondCounterColumnIndex));
			}
			result.close();
		}
		return gamingMap;
	...
	public static List<FillingMap> getMapFilledByID(final int levelID) throws SQLException {
		...
		if (callStatement.execute()) {
			...
			for (boolean isResultLeft = result.first(); isResultLeft; isResultLeft = result.next()) {
				objects.add(new FillingMap(result.getInt(XColumnIndex),
						result.getInt(YColumnIndex), result.getInt(IDMAPColumnIndex),result.getInt(TypeColumnIndex)));
			}
			result.close();
		}
		return objects;
